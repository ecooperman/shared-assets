// Single source of truth for inline SVG icons used across every app -
// change the markup here once and every <span data-icon="name"> across the
// fleet picks it up, instead of hunting down N copies. Originally built
// independently (and near-identically) in time-management/static/icons.js
// and trip-planning/static/icons.js; merged here as the shared version.
//
// Plain innerHTML injection rather than an external <svg><symbol>/<use>
// sprite, since older Safari has a history of flaky cross-document symbol
// references.
const ICONS = {
  compass:
  ...
  // Added for gifts' nav (replaces the 🎁 emoji).
  gift:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="20 12 20 22 4 22 4 12"></polyline><rect x="2" y="7" width="20" height="5"></rect><line x1="12" y1="22" x2="12" y2="7"></line><path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7z"></path><path d="M12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z"></path></svg>',
  // Added for gifts' Contacts nav link (replaces the 👤 emoji).
  person:
  ...
  // Added for time-management's Jobs nav (replaces the 💼 emoji).
  briefcase:
  ...
  // Added for time-management's Tags/Categories nav (replaces the 🏷️ emoji).
  tag:
  ...
  // Added for trip-planning's "Export" (.xlsx) button.
  download:
  ...
  // Added for trip-planning's agenda lock toggle (prevents accidental
  // drag-and-drop rescheduling while just browsing).
  lock:
  ...
  unlock:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 9.9-1"></path></svg>',
  // Added for the finances app nav (recurring items, transactions,
  // what-if scenarios, settings).
  repeat:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="17 1 21 5 17 9"></polyline><path d="M3 11V9a4 4 0 0 1 4-4h14"></path><polyline points="7 23 3 19 7 15"></polyline><path d="M21 13v2a4 4 0 0 1-4 4H3"></path></svg>',
  wallet:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 12V7H5a2 2 0 0 1 0-4h14v4"></path><path d="M3 5v14a2 2 0 0 0 2 2h16v-5"></path><path d="M18 12a2 2 0 0 0 0 4h4v-4z"></path></svg>',
  sliders:
...
// Exposed (not run-once-and-forgotten) because pages often build a lot of
// their DOM dynamically after load - anything that creates a `data-icon`
// element after the initial pass needs to call this again on it, or the
// span stays empty.
